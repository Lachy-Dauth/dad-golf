...
> Shipped. Group activity feed with likes/comments, privacy controls
> (private / in group), and 7 event types (round completions, new members,
> scheduled rounds, competition wins, handicap changes, badges earned).
> Accessible from main nav. Simple chronological feed — no algorithm.
...
> competition). Awarded automatically on round completion via server-side
> evaluation. Displayed on user profile page (`/user/:username`) visible
> to group members.
