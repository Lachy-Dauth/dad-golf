...
export async function seedDemoData(admin: AdminUser) {
  const counts = {
    users: 0,
    courses: 0,
    groups: 0,
  ...
  if (!alreadySeededRounds && courses.length >= 5) {
    ...
    // === STEP 6: IN-PROGRESS ROUND ===
    {
      ...
      const players = [fakeUsers[1], fakeUsers[3], fakeUsers[7]];
      const roomCode = await uniqueRoomCode();
      const round = await createRound(roomCode, course.id, group.id, players[0].id);
      const playerIds: string[] = [];
      for (const pu of players) {
    ...
    // === STEP 7: WAITING ROUND ===
    {
      ...
      const players = [fakeUsers[0], allUsers[8]];
      const roomCode = await uniqueRoomCode();
      const round = await createRound(roomCode, course.id, group.id, players[0].id);
      for (const pu of players) {
    ...
    const scheduledDefs: Array<{
      ...
      notes: string | null;
      cancel: boolean;
      rsvps: Array<[number, "accepted" | "declined" | "tentative"]>;
    }> = [
      {
        groupIdx: 0,
        courseIdx: 0,
    ...
    for (const sd of scheduledDefs) {
      ...
      const creatorIdx = [0, 1, 2][sd.groupIdx];
